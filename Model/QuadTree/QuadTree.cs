...
namespace TankMonogame.Model.QuadTree
{
    namespace TankMonogame.Model.QuadTree
    {
            ...
            where T : IGetBox<T>, IEquals<T>
        {
            ...
            public IEnumerable<BoxQT> FindMinQuad()
            {
                return findMinQuad(mRoot, mBox);
            }

            private IEnumerable<BoxQT> findMinQuad(Node node, BoxQT box)
            {
                if (node == null)
                    yield break;
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    {
                        foreach (var childBoxWithValues in findMinQuad(node.Children[i], ComputeBox(box, i)))
                        {
                            yield return childBoxWithValues;
                        }
                    }
                }
            }
